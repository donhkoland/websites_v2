...
(() => {
  ...
  const names={...Object.fromEntries(D.sections.map(s=>[s.k,s.n])),home:'Your stay',info:'Guest Information',dining:'Taste',taste:'Taste',experiences:'Experiences',wellness:'Wellbeing',wellbeing:'Wellbeing',explore:'Explore Vik'};
  ...
  function home(){
    ...
    const p=prop();
    const films={info:['pillar-hospitality','pillar-hospitality'],dining:['dining-cielomar','dining-cielomar'],taste:['dining-cielomar','dining-cielomar'],experiences:['ridegauchos','@exp-gauchos'],wellness:['shack','@exp-rest'],wellbeing:['shack','@exp-rest'],explore:['pillar-art','pillar-art']};
  ...
  function propertyServices(){return D.services.map(([title,text])=>[title,title==='Housekeeping'&&prop().housekeeping?prop().housekeeping:text]);}
  function propertyLeaving(){return D.leaving.map(([title,text])=>[title,title==='Late Check-Out'&&prop().lateCheckout?prop().lateCheckout:title==='Breakfast To Go'&&prop().breakfastToGo?prop().breakfastToGo:text]);}
  function propertyKnow(){return D.know.map(([title,text])=>[title,title==='Lost & Found'&&prop().lostFound?prop().lostFound:text]);}
  ...
  function info(){
    const p=prop(),film='dest-'+state.property,poster=media+film+'-poster.webp';return `<header class="info-heading wrap"><p class="eyebrow">${escape(p.name)} · The essentials</p><h1 class="editorial">Guest<br><em>Information.</em></h1></header><div class="wrap info-layout">${wifi(true)}<div>${accordion('Dining Hours',hours(p.dining)+pending(p.notes.filter(x=>!x.includes('Wi-Fi')).join(' '))+'<div class="info-block"><h3>Special Dining Experiences</h3><p>Private dining, signature dinners and exclusive culinary experiences are available upon request.</p></div>'+`<a class="text-link" href="${route('taste')}">Explore Taste ${arrow}</a>`)}${accordion('Services',serviceBlocks(propertyServices()))}${accordion('Facilities',blocks(p.facilities))}${accordion('Before You Leave',blocks(propertyLeaving()))}${accordion('Good to Know',blocks(propertyKnow()))}${state.property==='bahia'?pending(p.notes.join(' ')):''}</div></div><figure class="media-break wrap"><div class="media-break-stage">${img(film,p.name)}<video data-film="${media+film}.mp4" poster="${poster}" data-poster="${poster}" preload="none" muted loop playsinline webkit-playsinline disablepictureinpicture aria-hidden="true" tabindex="-1"></video></div><figcaption>${escape(p.name)} · José Ignacio, Uruguay</figcaption></figure>${closing()}`;
  }
  const itemFilms={
    ...
    'Farm-to-Table Cooking Class':['pillar-dining','@exp-garden'],'Garzón Bike Tour & Bodega':[null,'@map-garzon'],"Vik Experience — Bodega Oceánica O'33":[null,'@exp-o33.webp'],'Art Exploration':['collection','@exp-collection'],'Cabo Polonio':[null,'@tour-cabo-polonio.webp'],'Punta del Este & Punta Ballena':[null,'@tour-punta-del-este.webp'],'Marimonte Summer Camp':[null,'spring'],
  ...
  function stay(){
    ...
    return `${sectionHero('stay')}<div class="wrap content modular-content"><p class="lede">Make yourself at home at ${escape(prop().name)}. Your daily essentials and everything included in your stay.</p>${rail('Included in your stay',cards)}${essentials()}${accordion('Services',serviceBlocks(propertyServices()))}${accordion('Dining Hours',hours(prop().dining))}<a class="book info-full-cta" href="${route('info')}">Guest Information · Services & practical details ${arrow}</a></div>${closing()}`;
  }
