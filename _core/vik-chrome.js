...
(() => {
  ...
  /* Una pagina con motor de reserva propio ancla dentro de si misma; el
     resto manda a la portada, que es donde vive. Misma regla que usa el
     lightbox de las tres casas, para que el pie no quede inerte en las
     paginas que no llevan formulario. */
  /* ── El boton de reserva de la barra ────────────────────────
     Dos formas de la misma accion: la frase entera mientras haya
     sitio, y una sola palabra en un telefono, donde "Book Your Stay"
     se parte en cuatro lineas o empuja la hamburguesa fuera de la
     pantalla. Cual se ve lo decide el CSS, no este archivo.

     El data-i18n baja al span y no al enlace: puesto en el enlace, el
     traductor reescribe todo el contenido y se lleva por delante los
     dos spans. */
  const bookLabels = S => {
    const long = S.bookLabel || "Book Your Stay";
    const short = S.bookShort || long.split(" ")[0];
    const key = S.bookLabel ? "" : ' data-i18n="cta.book"';
    const keyS = S.bookLabel ? "" : ' data-i18n="cta.bookShort"';
    return `<span class="nb-long"${key}>${long}</span>` +
           `<span class="nb-short"${keyS}>${short}</span>`;
  };

  const link = l => {
    const href = l.href === "#book" && !document.querySelector("#book")
      ? "../vik-jose-ignacio/#book" : l.href;
    /* data-stay marca los enlaces que hacen una pregunta en vez de
       nombrar un destino. vik-chooser los intercepta y abre la
       comparacion de las tres casas ahi mismo — el href se deja real,
       asi que sin JavaScript el enlace sigue llevando a la pagina. */
    return `<a href="${href}"${l.stay ? " data-stay" : ""}${l.ext ? ' target="_blank" rel="noopener"' : ""}>${l.label}</a>`;
  };
  ...
  <aside class="quick-menu" id="quickMenu" role="dialog" aria-modal="true" aria-hidden="true">
    ...
    <div class="menu-places" data-places>
      <button class="menu-places__t" type="button" aria-expanded="false" aria-controls="menuPlaces">
        <span class="k">The VIK world</span>
        <span class="c">${String(S.sub.length).padStart(2, "0")}</span>
        <span class="menu-places__sign" aria-hidden="true"></span>
      </button>
      <div class="menu-sub" id="menuPlaces">
        <div class="menu-sub__in">
          ${S.sub.map(link).join("\n          ")}
        </div>
      </div>
    </div>
    ...
    <div class="menu-foot">
      <a class="menu-book" href="${S.bookHref || BOOK}"${S.bookHref ? "" : ' target="_blank" rel="noopener"'}><span${S.bookLabel ? "" : ' data-i18n="cta.book"'}>${S.bookLabel || "Book Your Stay"}</span><span class="arw">&#8594;</span></a>
      <div class="menu-langs">
        <span class="menu-langs__k">Language</span>
        <div class="lang-menu">
          <button type="button" data-lang="en" class="active"><span class="ln">English</span><span class="lc">EN</span></button>
          <button type="button" data-lang="es"><span class="ln">Español</span><span class="lc">ES</span></button>
          <button type="button" data-lang="pt"><span class="ln">Português</span><span class="lc">PT</span></button>
        </div>
      </div>
    </div>
  ...
  <nav class="nav" id="mainNav" aria-label="Primary">
    <div class="nav-left">
      ...
      ${mark === LOGO
        ...
        : `<a class="nav-tag" href="../vik-retreats/" aria-label="VIK Retreats · the group">VIK Retreats</a>`}
    </div>
    <div class="nav-end">
      <a class="nav-link nav-book" href="${S.bookHref || BOOK}"${S.bookHref ? "" : ' target="_blank" rel="noopener"'}>${bookLabels(S)}</a>
  ...
  <form class="bookbar" id="bookbar" data-booking>
    ...
    <div class="bookbar-actions">
      <a class="btn-line" href="../contact/" data-i18n="cta.journey">Contact a Journey Designer</a>
  ...
  const retreats = () => {
    ...
    const book = document.querySelector("#book") ? "#book" : "../vik-jose-ignacio/#book";
    return `
  ...
  const bookLightbox = S => {
    ...
    /* Por defecto coincide el host exacto de la ficha. Una direccion
       con varias mesas — cuatro restaurantes, cuatro direcciones dentro
       del mismo motor — declara bookHost y coincide el dominio entero. */
    const host = S.bookHost || (url.match(/^https?:\/\/([^/]+)/i) || [])[1] || "";
  ...
  <div class="rlb" id="bookLb" role="dialog" aria-modal="true"
    ...
    <div class="rlb__panel">
      <div class="rlb__hd">
        <span class="rlb__t" id="bookLbT">${S.bookLabel || "Book"}</span>
    ...
    const title = document.getElementById("bookLbT");
    const fb = lb.querySelector(".rlb__fb a");
    let cur = null, last = null;
    frame.addEventListener("error", () => lb.classList.add("is-blocked"));

    /* Se abre con la direccion que pidio el enlace. Sin ninguna, la de
       la ficha, que es el caso de una direccion con una sola mesa. El
       iframe solo vuelve a cargar cuando la direccion cambia. */
    const open = (u, label) => {
      const want = /^https?:/i.test(u || "") ? u : url;
      ...
      if (cur === want) return;
      cur = want;
      lb.classList.remove("is-blocked");
      frame.src = want;
      if (fb) fb.href = want;
    };
    const close = () => {
    ...
    document.addEventListener("click", e => {
      ...
      if (host && href.indexOf(host) < 0) return;
      e.preventDefault();
      open(href, a.dataset.bookLabel || "");
    });
  ...
  /* ── El desplegable de lugares ──────────────────────────────
     Ocho direcciones dentro del menu son ocho lineas que compiten con
     las ocho secciones y ganan por peso visual sin ganar en jerarquia.
     Plegadas detras de un titulo, el menu vuelve a leerse: primero
     donde ir, y despues, si hace falta, a que casa.

     Abre y cierra con grid-template-rows, que es lo unico que anima
     de alto automatico sin numeros magicos. El estado vive en
     aria-expanded y la hoja de estilo lee de ahi: una sola verdad. */
  const mountPlaces = () => {
    const box = document.querySelector("[data-places]");
    if (!box) return;
    const btn = box.querySelector(".menu-places__t");
    const panel = box.querySelector(".menu-sub");
    if (!btn || !panel) return;
    ...
    /* Al cerrar el menu se pliega, para que la proxima vez que se abra
       empiece donde empezo la primera. */
    const menu = document.getElementById("quickMenu");
    if (menu && window.MutationObserver)
      new MutationObserver(() => {
        if (!menu.classList.contains("is-open")) set(false);
      }).observe(menu, { attributes: true, attributeFilter: ["class"] });

    /* El menu se cierra al tocar la reserva. El panel de reservas vive
       mas abajo en la pila — tiene que estarlo, o taparia al menu en
       todas las demas pantallas — asi que si el menu sigue abierto el
       motor se abre detras de el y no se ve nada. Cerrarlo con el
       propio boton mantiene el estado de vik-motion en su sitio. */
