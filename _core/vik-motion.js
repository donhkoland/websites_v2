...
(() => {
  ...
  const syncVW = () => {
    ...
    /* Con un panel abierto el cuerpo deja de desplazarse, la barra de
       scroll desaparece y el ancho del cliente crece 15px. Si --vw
       sigue ese salto, al cerrar el panel cada modulo a sangre queda
       15px mas ancho que la ventana hasta que el vigilante corrige, y
       la pagina se va de lado por medio segundo. Mientras hay panel,
       --vw se congela: un cambio de tamano con el panel abierto no es
       un caso que valga la pena servir. */
    const locked = () => document.body.classList.contains("is-locked") ||
                         document.body.classList.contains("plb-open") ||
                         document.body.style.overflow === "hidden";
    const set = () => { if (!locked()) el.style.setProperty("--vw", el.clientWidth + "px"); };
    set();
  ...
  const derivative = (rel, cssPx) => {
    if (!rel) return null;
    /* Una ruta ya resuelta se devuelve tal cual. Empieza por assets/
       cuando los assets estan al lado, y por la base cuando estan en
       otra carpeta — las dos son rutas terminadas. */
    const done = window.VIK_BASE || "assets/";
    if (rel.startsWith(done) || rel.startsWith("assets/") || rel.startsWith("http"))
      return { src: rel, fallback: rel };
    const need = cssPx * Math.min(window.devicePixelRatio || 1, 2);
    const w = SIZES.find(s => s >= need) || SIZES[SIZES.length - 1];
    const slug = rel.replace(/\//g, "_").replace(/\.[^.]+$/, "");
    return { src: A(`assets/img/t/${slug}-w${w}.jpg`), fallback: A(`assets/img/${rel}`) };
  };
  ...
  const setupIndexThumbs = () => {
    ...
    $$(".index-row[data-hover-img]").forEach(row => {
      ...
      im.decoding = "async";
      im.addEventListener("error", () => {
        if (im.src.indexOf("/t/") > -1) im.src = A("assets/img/") + rel;
      }, { once: true });
